...
try {
  ...
  const tracks = db
    .prepare('SELECT title, artist, rating, loved, play_count FROM tracks LIMIT 10')
    .all()
  ...
  columns.forEach((col) => {
    if (
      col.name.includes('play') ||
      col.name.includes('count') ||
      col.name.includes('rating') ||
      col.name.includes('loved')
    ) {
