...
try {
    ...
    const scrobbles = db.prepare("SELECT * FROM scrobble_queue ORDER BY played_at DESC LIMIT 10").all();
    ...
    const pending = db.prepare("SELECT COUNT(*) as count FROM scrobble_queue WHERE lastfm_submitted = 0 OR listenbrainz_submitted = 0").get();
