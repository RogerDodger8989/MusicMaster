...
export function addTrackArtist(
  ...
  try {
    ...
    stmt.run(uuidv4(), trackId, artistId, role, instrument, creditedAs, joinPhrase, sortPosition)
...
export function addAlbumArtist(
  ...
  try {
    ...
    stmt.run(uuidv4(), albumId, artistId, role, creditedAs, joinPhrase, sortPosition)
...
export function addExternalLink(
  ...
  try {
    ...
    stmt.run(uuidv4(), entityType, entityId, linkType, url, description)
...
export function addExternalIdentifier(
  ...
  try {
    ...
    stmt.run(uuidv4(), entityType, entityId, identifierType, value)
...
export function storeAcousticBrainzData(
  ...
  try {
    ...
    stmt.run(
      ...
      data.valence || null,
      data.mood_acoustic || null,
      data.mood_aggressive || null,
      data.mood_electronic || null,
      data.mood_happy || null,
      data.mood_sad || null,
      data.mood_relaxed || null,
      data.mood_party || null
    )
 ...
 */
export function getTrackArtists(trackId: string): (DbTrackArtist & { artist: DbArtist })[] {
  try {
    const db = getDatabase()
    const artists = db
      .prepare(
 ...
 */
export function getTrackAcousticBrainzData(trackId: string): DbAcousticBrainzData | null {
  try {
    const db = getDatabase()
    const data = db.prepare('SELECT * FROM acousticbrainz_data WHERE track_id = ?').get(trackId) as
      | DbAcousticBrainzData
      | undefined
    return data || null
  } catch (error) {
...
export function searchTracksByISRC(isrc: string): DbTrack[] {
  try {
    const db = getDatabase()
    const tracks = db.prepare('SELECT * FROM tracks WHERE isrc = ?').all(isrc) as DbTrack[]
    return tracks || []
  } catch (error) {
