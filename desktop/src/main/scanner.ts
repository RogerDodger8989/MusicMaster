...
export class MusicScanner extends EventEmitter {
  ...
  async scanFolder(folderId: string, folderPath: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Scan complete: ${this.scanProgress.scannedFiles}/${this.scanProgress.totalFiles} files processed`
      )
    } finally {
    ...
    try {
      ...
      if (ratingTag) {
        ...
        if (rawRating >= 1) {
          ...
          console.log(
            `[Scanner] ${path.basename(filePath)} Raw rating ${rawRating} (scale ${max}) -> ${rating} stars`
          )
        } else if (rawRating > 0) {
          // Rating between 0-1: already normalized, convert to 0-5
          rating = Math.round(rawRating * 5)
          console.log(
            `[Scanner] ${path.basename(filePath)} Normalized rating ${rawRating} -> ${rating} stars`
          )
        }

        if (
          ratingObj.source?.toLowerCase().includes('loved') ||
      ...
      if (!loved && !existingTrack?.loved) {
        const nativeLoved = metadata.native?.vorbis?.find((t) =>
          ['LOVED', 'HEART', 'FAVORITE'].includes(t.id.toUpperCase())
        )
        if (nativeLoved) {
