...
export class ListenBrainzService {
  ...
  async submitListen(payload: ListenBrainzListenPayload, timestamp?: number): Promise<boolean> {
    ...
    console.log(
      '🎵 Submitting listen to ListenBrainz, token:',
   ...
   */
  async submitListens(
    payloads: Array<{ payload: ListenBrainzListenPayload; timestamp?: number }>
  ): Promise<boolean> {
    const currentToken = getToken()
    if (!currentToken) {
    ...
    try {
      ...
      if (matchingRecording) {
        ...
        recordings.slice(0, 5).forEach((rec: any, i: number) => {
          console.log(
            `   ${i + 1}. "${rec.track_metadata?.artist_name || '?'}" - "${rec.track_metadata?.track_name || '?'}" (${rec.listen_count} listens)`
          )
        })
        return 0
      }
   ...
   */
  async fetchAllListens(
    username: string,
    onProgress?: (stats: { fetched: number; total?: number; page: number }) => void
      ...
      while (retries <= maxRetries && !success) {
        try {
          ...
          if (error.response?.status === 429) {
            ...
            console.warn(
              `[ListenBrainz] 🛑 Rate limit exceeded (429) on page ${page}. Waiting ${waitTime}s to reset...`
            )
          ...
          if (retries > maxRetries) {
            console.error(
              `[ListenBrainz] ❌ Final failure fetching page ${page} after ${maxRetries} retries:`,
              error
            )
            throw error
          }
          ...
          console.warn(
            `[ListenBrainz] ⚠️ Error fetching page ${page} (attempt ${retries}). Retrying in ${delay}ms...`,
            error.message
          )
