...
export function calculateMatchScore(
    ...
    const albumScore = localAlbumNorm && mbAlbumNorm
        ? stringSimilarity(localAlbumNorm, mbAlbumNorm)
        : 0.5 // Neutral if no album
...
export async function advancedMatch(
    ...
    const textResults = await searchFn(
        localArtist,
        localTitle,
        localAlbum
    )
