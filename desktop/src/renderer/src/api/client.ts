...
export class DomClient implements MusicClient {
    ...
    async updateTrack(id: string, updates: Partial<Track>): Promise<void> {
        ...
        if (updates.playCount !== undefined) await this.api.tracks.updatePlayCount(id, updates.playCount)
    }
    ...
    async createPlaylist(name: string, trackIds: string[]): Promise<any> { return null }
    ...
    async createLastFmSession(): Promise<any> { return {} }

    async syncMetadata(): Promise<void> { }
    async getFileSyncStatus(): Promise<any> { return {} }
    async enhanceLibrary(): Promise<void> { }
    async getEnhanceStatus(): Promise<any> { return {} }
    async getCoverage(): Promise<any> { return {} }
    async writeTrackMetadata(id: string): Promise<boolean> { return false }
    ...
    async searchAlbums(artist: string, album: string): Promise<any[]> { return [] }
    ...
    async getDirectory(path: string): Promise<any[]> { return [] }

    getCoverUrl(id: string): string { return '' }
    getArtistImageUrl(id: string): string { return '' }
    getAudioUrl(id: string): string { return '' }
}
...
export class RestClient implements MusicClient {
    ...
    async createPlaylist(name: string, trackIds: string[]): Promise<any> {
        return this.req<any>('/api/playlists', { method: 'POST', body: JSON.stringify({ name, trackIds }) })
    }
    ...
    async addToPlaylist(id: string, trackId: string): Promise<void> {
        await this.req(`/api/playlists/${id}/tracks`, { method: 'POST', body: JSON.stringify({ trackId }) })
    }

    async removeFromPlaylist(id: string, trackId: string, position: number): Promise<void> {
        await this.req(`/api/playlists/${id}/tracks/${trackId}?position=${position}`, { method: 'DELETE' })
    }
    ...
    async scrobble(artist: string, track: string, album?: string, duration?: number, timestamp?: number): Promise<void> {
        await this.req('/api/scrobble', { method: 'POST', body: JSON.stringify({ artist, track, album, duration, timestamp }) })
    }

    async updateNowPlaying(artist: string, track: string, album?: string, duration?: number): Promise<void> {
        await this.req('/api/scrobble/nowplaying', { method: 'POST', body: JSON.stringify({ artist, track, album, duration }) })
    }
...
export const client: MusicClient = useServer
    ? new RestClient(SERVER_URL)
    : new DomClient()
