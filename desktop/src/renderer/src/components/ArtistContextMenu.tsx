...
export default function ArtistContextMenu({ artist, x, y, onClose }: ArtistContextMenuProps) {
    ...
    const handlePlayNext = () => {
        const tracks = getArtistTracks()
        window.dispatchEvent(new CustomEvent('request-artist-play', {
    ...
    return (
        ...
        >
            <div className="px-4 py-2 mb-1 border-b border-zinc-800/50">
                <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest truncate">{artist.name}</div>
                <div className="text-[10px] text-zinc-600 truncate">{artist.albumCount} Albums • {artist.trackCount} Tracks</div>
            </div>
