...
export const FileBrowserModal: React.FC<FileBrowserModalProps> = ({
  ...
  const handleUpClick = () => {
    ...
    if (
      drives.some((d) => d.path === currentPath) ||
        ...
        <div className="flex-1 overflow-auto bg-zinc-900 min-h-[300px]">
          ...
          {error && (
            <div className="flex flex-col items-center justify-center h-full text-red-400 p-4">
              ...
              <p>{error}</p>
              <button
                onClick={() => (currentPath ? loadDirectory(currentPath) : loadDrives())}
          ...
          {!loading && !error && (
            <div className="flex flex-col text-sm border-t border-zinc-800">
              ...
              {currentPath === '' &&
                drives.map((drive) => (
                  ...
                  >
                    ...
                    <span className="flex-1 font-medium text-zinc-200 group-hover:text-white">
                      {drive.name}
                    </span>
              ...
              {currentPath !== '' && (
                <>
                  {nodes.length === 0 && (
                    <div className="p-8 text-center text-zinc-500 italic">This folder is empty</div>
                  )}
                  {nodes.map((node) => (
                    <button
                      key={node.path}
