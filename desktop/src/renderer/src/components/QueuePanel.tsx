import { X, ListMusic, Save, Trash2, Shuffle, GripVertical, Play, Pause, Star, Heart, Clock } from 'lucide-react'
...
export default function QueuePanel({ isOpen, width, onClose }: QueuePanelProps) {
    const {
        ...
        playFromQueueAndCleanup, history, reorderQueue, insertToQueue
    } = usePlayer()
    ...
    return (
        <div
            ...
            className="h-full bg-zinc-950 border-l border-zinc-800 flex flex-col relative z-20"
        >
            {/* Header / Tabs */}
            <div className="flex border-b border-zinc-800">
                <button
                    ...
                    className={cn(
                        ...
                        activeTab === 'queue' ? "text-blue-500 border-b-2 border-blue-500" : "text-zinc-500 hover:text-zinc-300"
                    )}
                >
                    Queue ({queue.length})
                </button>
                <button
                    ...
                    className={cn(
                        "flex-1 py-4 text-xs font-black uppercase tracking-widest transition-colors",
                        activeTab === 'played' ? "text-blue-500 border-b-2 border-blue-500" : "text-zinc-500 hover:text-zinc-300"
                    )}
                >
                    Played
                </button>
            ...
            >
                {(activeTab === 'queue' ? queue : history).length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center p-8 text-center text-zinc-700 space-y-4">
                        ...
                        <p className="text-sm font-medium">No tracks {activeTab === 'queue' ? 'in queue' : 'played yet'}</p>
                    </div>
                ) : (
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {(() => {
                                                const coverPath = track.coverArtPath || albums.find(a => a.name === track.album && a.artist === (track.albumArtist || track.artist))?.coverArtPath
                                                if (coverPath) {
                                                    return (
                                                        <img
                                                            src={coverPath && coverPath.startsWith('asset:') ? coverPath : `asset:///${coverPath?.replace(/\\/g, '/')}`}
                                        ...
                                        <div className="flex-1 min-w-0 space-y-0.5">
                                            ...
                                            <div className="flex items-center gap-3 pt-0.5 transition-opacity">
                                                <div className="flex">
                                                    {[1, 2, 3, 4, 5].map(s => (
                                                        <Star
                                                            key={s}
                                                            size={10}
                                                            className={cn("cursor-pointer", s <= track.rating ? "text-yellow-500 fill-current" : "text-zinc-800 hover:text-zinc-700")}
                                                            onClick={(e) => {
                                                                e.stopPropagation()
                                                ...
                                                <Heart
                                                    size={10}
                                                    className={cn("cursor-pointer transition-colors", track.loved ? "text-red-500 fill-current" : "text-zinc-800 hover:text-red-500/50")}
                                                    onClick={(e) => {
                                                        e.stopPropagation()
                ...
                showClearConfirm && (
                    <div className="absolute inset-0 z-50 flex items-center justify-center p-6 text-center">
                        ...
                        >
                            ...
                            <p className="text-zinc-500 text-xs mb-6 leading-relaxed">This will remove all tracks from your playback queue. This cannot be undone.</p>
                                        ...
                                        {playlists.map(pl => (
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="text-[10px] text-zinc-600 font-medium">{pl.tracks.length} tracks</div>
                                                </div>
                                            </button>
                                        ))}
                                    </div>
