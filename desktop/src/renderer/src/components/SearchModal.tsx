...
export default function SearchModal() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        e.preventDefault()
        setSelectedIndex(
          (prev) => (prev - 1 + flatResults.length) % Math.max(1, flatResults.length)
        )
      } else if (e.key === 'Enter') {
        e.preventDefault()
  ...
  const handleSelect = (type: string, item: any) => {
    ...
    if (type === 'artist') {
      ...
      if (item.albumId) {
        navigateTo('album-detail', { albumId: item.albumId })
      } else {
        // Fallback: search for album by name? Or just play the track?
        // For now, let's just log or play. 
        // Better to just play it if we can't find the album.
      ...
      >
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 custom-scrollbar">
          {query.trim().length < 2 ? (
            ...
            <div className="p-8 text-center text-zinc-500 animate-pulse">Searching library...</div>
          ) : (
                  ...
                  <div className="space-y-1">
                    {results.artists.map((artist, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div className="w-10 h-10 rounded-full bg-zinc-800 overflow-hidden flex items-center justify-center text-zinc-500 group-hover:bg-zinc-700 transition-colors">
                            {artist.imagePath ? (
                              <img
                ...
                <section>
                  ...
                  <div className="space-y-1">
                    {results.tracks.map((track, idx) => {
                      const isSelected =
                        selectedIndex === results.artists.length + results.albums.length + idx
                      return (
                        <button
                          key={track.id}
                        ...
                        >
                          <div className="w-10 h-10 rounded bg-zinc-800 overflow-hidden flex items-center justify-center text-zinc-500 group-hover:bg-zinc-700 transition-colors">
                            {track.albumId ? (
                              <img
                                src={client.getCoverUrl(track.albumId)}
                                alt={track.album}
                                className="w-full h-full object-cover"
                                onError={(e) => {
                                  ; (e.target as HTMLImageElement).style.display = 'none'
