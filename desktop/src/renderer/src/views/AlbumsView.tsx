...
export default function AlbumsView({ onAlbumClick }: AlbumsViewProps) {
  ...
  const filteredAlbums = useMemo(() => {
    ...
    // Filter by Genre
    if (selectedGenre) {
      result = result.filter((album) => album.genre?.split(' / ').includes(selectedGenre))
    }
  ...
  const genres = useMemo(() => {
    ...
    albums.forEach((album) => {
      ...
      const parts = album.genre
        .split(' / ')
        .map((g) => g.trim())
        .filter((g) => g && g.toLowerCase() !== 'unknown')
      parts.forEach((p) => {
  ...
  const handlePlayAll = async () => {
    ...
    if (albumTracks && albumTracks.length) {
      albumTracks.sort(
        (a, b) => (a.discNum || 1) - (b.discNum || 1) || (a.trackNum || 0) - (b.trackNum || 0)
      )
        ...
        {filteredAlbums.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Try adjusting your filters or scan for music in Settings.
              </p>
            </div>
            {selectedGenre && (
              <button
          ...
          <>
            {viewMode === 'list' ? (
              ...
              <div
                className={cn('grid gap-6 pb-8', 'grid-cols-[repeat(auto-fill,minmax(180px,1fr))]')}
              >
                {filteredAlbums.map((album) => (
                  <AlbumCard
                    key={album.id}
