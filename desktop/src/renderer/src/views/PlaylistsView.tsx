...
export default function PlaylistsView() {
  ...
  const { selectedTracks, handleTrackClick, clearSelection, selectSingleTrack, setSelectedTracks } =
  ...
  if (selectedPlaylist) {
    return (
      <div className="p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500 relative min-h-full">
          ...
          <div className="flex-1 space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-zinc-500 font-medium">
                ...
                {formatDuration(selectedPlaylist.tracks.reduce((acc, t) => acc + t.duration, 0))}
              </p>
            </div>
        ...
        {/* Track List */}
        <div
          className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden focus:outline-none"
          tabIndex={0}
        >
                ...
                >
                  <div className="text-center text-xs font-bold text-zinc-600">
                    {isActive && isPlaying ? (
                      ...
                      <span className={cn(isActive || isSelected ? 'text-blue-500' : '')}>
                        {idx + 1}
                      </span>
                    )}
                  </div>
                  <div className="min-w-0">
                    <div
                      className={cn(
                        ...
                        isActive || isSelected ? 'text-blue-500' : 'text-zinc-200'
                      )}
                    >
                      {track.title}
                    </div>
        ...
        {showDeletePlaylistConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 text-center">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 shadow-2xl relative w-full max-w-[320px] animate-in zoom-in-95 duration-200">
              ...
              <p className="text-zinc-500 text-sm mb-6 leading-relaxed">
                This will permanently delete <strong>{selectedPlaylist.name}</strong>. This cannot
                be undone.
              </p>
              <div className="flex gap-3">
                <button
