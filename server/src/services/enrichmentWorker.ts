    ...
    try {
      ...
      if (release.media && release.media[0]?.tracks) {
        ...
        if (matchedTrack?.recording?.id) {
          ...
          db.prepare('UPDATE tracks SET musicbrainz_recording_id = ? WHERE id = ?').run(mbid, trackId)
        }
      }
    } catch (error) {
...
// Count all tracks in the current DB
    ...
    for (const [albumMbid, trackIds] of albumGroups) {
      ...
      for (const trackId of trackIds) {
        const track = db.prepare('SELECT musicbrainz_recording_id FROM tracks WHERE id = ?').get(trackId) as any
        
        // Pass null recording_id if not present - enrichTrackAcousticBrainz will try to fetch it
        const enriched = await enrichTrackAcousticBrainz(trackId, track?.musicbrainz_recording_id || null, progress)
        if (enriched) {
